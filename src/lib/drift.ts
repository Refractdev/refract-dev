...
function analyzeCategoryTrends(snapshots: SnapshotData[]): CategoryTrend[] {
  // Gather all category names across all snapshots
...
function detectAnomalies(snapshots: SnapshotData[]): Anomaly[] {
  ...
  for (const category of allCategories) {
    ...
    if (sd < 0.5) continue // no meaningful deviation if all values are ~same
...
function findDecayHotspots(snapshots: SnapshotData[]): DecayHotspot[] {
  ...
  // Collect all files seen across snapshots
  const allFiles = new Map<string, number[]>() // filePath → counts per snapshot
  ...
  for (const snap of snapshots) {
    ...
    for (const [filePath, count] of Object.entries(snap.file_issue_counts)) {
      ...
      // Pad with 0 for snapshots where this file had no issues
    }
  }

  // Fill in the arrays, ensuring all snapshots are represented
  ...
  // Compute overall project growth rate
  ...
  for (const [filePath, counts] of allFiles) {
    ...
    // Only flag if the file has appeared in multiple snapshots and has non-zero issues
    if (appearances < 2 || latestCount === 0) continue

    // Flag if file slope is significantly worse than project slope
